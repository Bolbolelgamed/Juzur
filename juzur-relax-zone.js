...
function scheduleHeroAutoplay(){
  ...
  heroPhotoIdleTimer=setTimeout(()=>{
    heroPhotoTimer=setInterval(()=>showHeroPhoto(heroPhotoIndex+1),HERO_PHOTO_ROTATION_MS);
...
if(inlineOrderCtas.length){
  if('IntersectionObserver' in window){
    const inlineOrderCtaObserver=new IntersectionObserver(()=>setInlineOrderCtaVisibilityState(),{threshold:.01});
