...
export default function Hero() {
  ...
  return <section className="hero hero-carousel" aria-label={t.hero.label}>
    ...
    <div className="hero-product-gallery reveal delay" id="heroGallery">
      <div className="hero-main-photo"><video id="heroVideo" className="hero-gallery-video is-visible" src={`${assetsBase}juzur-hero-video.mp4`} muted loop controls playsInline preload="none" poster={`${assetsBase}new-photos/juzur-photo-08.webp`} aria-label={t.hero.videoAlt} /><img id="heroPreviousImage" className="hero-gallery-layer hero-gallery-layer-previous" src={`${assetsBase}new-photos/juzur-photo-08.webp`} alt="" aria-hidden="true" width="1484" height="1113" style={{ objectPosition: '50% 50%' }} /><img id="heroCurrentImage" className="hero-gallery-layer hero-gallery-layer-current is-visible" src={`${assetsBase}new-photos/juzur-photo-08.webp`} alt={t.images.top} width="1484" height="1113" fetchPriority="high" decoding="async" style={{ objectPosition: '50% 50%' }} /></div>
      <div className="hero-photo-thumbs" aria-label={t.hero.mediaLabel}><button className="hero-photo-thumb hero-media-thumb active" type="button" data-kind="video" aria-label={t.hero.videoPlay}><img src={`${assetsBase}new-photos/juzur-photo-08-480.webp`} alt="" width="480" height="343" loading="lazy" decoding="async" /></button>{heroPhotos.map(([src, alt, position]) => <button key={src} className="hero-photo-thumb hero-media-thumb" type="button" data-kind="image" data-img={src} data-alt={alt} data-position={position} aria-label={alt}><img src={thumbnailImage(src)} alt="" width="480" height="343" loading="lazy" decoding="async" style={{ objectPosition: position }} /></button>)}</div>
