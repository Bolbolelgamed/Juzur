...
export default function Hero() {
  ...
  const heroPhotos = [['/assets/juzur-tray-studio.jpg', t.images.studio, '50% 50%'], ['/assets/product-2.jpg', t.images.side, '50% 50%'], ['/assets/juzur-tray-lifestyle.jpg', t.images.lifestyle, '50% 72%'], ['/assets/product-4.jpg', t.images.top, '50% 50%']];
  return <section className="hero hero-carousel" aria-label={t.hero.label}>
    ...
    <div className="hero-product-gallery reveal delay" id="heroGallery">
      <div className="hero-main-photo"><video id="heroVideo" className="hero-gallery-video is-visible" src="/assets/juzur-hero-video.mp4" autoPlay muted loop playsInline preload="auto" aria-label={t.hero.videoAlt} /><img id="heroPreviousImage" className="hero-gallery-layer hero-gallery-layer-previous" src="/assets/juzur-tray-studio.jpg" alt="" aria-hidden="true" style={{ objectPosition: '50% 50%' }} /><img id="heroCurrentImage" className="hero-gallery-layer hero-gallery-layer-current" src="/assets/juzur-tray-studio.jpg" alt={t.images.studio} fetchPriority="high" style={{ objectPosition: '50% 50%' }} /></div>
      <div className="hero-photo-thumbs" aria-label={t.hero.mediaLabel}><button className="hero-photo-thumb hero-media-thumb active" type="button" data-kind="video" aria-label={t.hero.videoPlay}><video src="/assets/juzur-hero-video.mp4" muted playsInline preload="metadata" aria-hidden="true" /></button>{heroPhotos.map(([src, alt, position], index) => <button key={src} className="hero-photo-thumb hero-media-thumb" type="button" data-kind="image" data-img={src} data-alt={alt} data-position={position} aria-label={alt}><img src={src} alt="" loading={index === 0 ? 'eager' : 'lazy'} decoding="async" style={{ objectPosition: position }} /></button>)}</div>
      ...
      <div className="hero-price" aria-label={t.hero.priceLabel}><div><span>{t.hero.special}</span><strong><bdi>{formatPrice(product.finalUnitPrice, language)}</bdi></strong></div></div>
